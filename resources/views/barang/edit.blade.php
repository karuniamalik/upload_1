            ...
            <div class="col">
                ...
                <form action="{{ url('barang/' . $data->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    name="kategori_id"> --}}
                        ...
                        {{-- <select class="custom-select " name="details_ategory">

                            <option selected>Pilih Kategori</option>
                            @foreach ($kategori as $ktgr)
                                <option value="{{ $ktgr->id }}"
                                    {{ $ktgr->id == $data->kategori_id ? 'selected' : '' }}>{{ $ktgr->kategori }}</option>
                            @endforeach
                        </select> --}}
                        ...
                        <select class=" form-control js-example-basic-multiple" name="kategori[]" multiple="multiple">
                            @foreach ($kategori as $ktgr)
                                <option value="{{ $ktgr->id }}"
                                    @foreach ($dataDetailKategori as $ktg_id)
                                        {{ $ktgr->id == $ktg_id->kategori_id ? 'selected' : '' }} @endforeach>
                                    {{ $ktgr->kategori }}
                                </option>
                            @endforeach
                        </select>
